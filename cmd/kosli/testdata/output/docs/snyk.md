...
|    -D, --dry-run  |  [optional] Run in dry-run mode. No data is sent to Kosli and the CLI exits with 0 exit code regardless of any errors. A [boolean flag](https://docs.kosli.com/faq/#boolean-flags).  |
...
For exposition, all flags are written explicitly. In practice, common flags, such as `--api-token`, `--org` are often provided via [environment-variables or config-files](/getting_started/install/#assigning-flags-via-environment-variables).
